...
void KaipuleSensor::setup() {
  // The unit of measurement is applied by the Python sensor platform
  // (sensor.py) through the entity string pool; the 2026.8.x Sensor
  // class has no C++ set_unit_of_measurement setter.
  auto *parent = this->parent_;
  parent->add_subscriber(this->mac_, [this](const DeviceState &state) {
    float value = NAN;
